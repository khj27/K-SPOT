...
function setup({ configured = true, fail = false, onRead = () => {} } = {}) {
  const exports = {};
  const docs = [
  ...
  const overrides = {
    ...
    '@/lib/firebase/admin': { isFirebaseAdminConfigured: () => configured, getFirebaseAdminDb: () => ({ collection: () => ({ limit: () => ({ get: async () => { onRead(); if (fail) throw new Error(); return { docs }; } }) }) }) },
...
test('concurrent and repeated public reads reuse one database query', async () => {
  let reads = 0;
  const api = setup({ onRead: () => reads++ });
  const results = await Promise.all([api.getPublicExploreContents(), api.getPublicExploreContents()]);
  await api.getPublicExploreContents();
  assert.equal(reads, 1); assert.equal(results[0][0].id, 'registered');
});
test('missing configuration or database outage never introduces demo places', async () => {
  ...
  await assert.rejects(setup({ fail: true }).getPublicExploreContents());
});
