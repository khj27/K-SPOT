...
export function AccountPanel({ children }: { user: UserIdentity | null; children?: ReactNode }) {
  ...
  return <div className="account-layout">
    <section className="account-profile"><div className="account-avatar" aria-hidden="true">{(user.name || "회원").slice(0, 1)}</div><div><p className="kspot-eyebrow">MY ACCOUNT</p><h2><LocaleText>{user.name || "회원"}</LocaleText><LocaleText>{"님의 계정"}</LocaleText></h2><p className="account-email">{user.email}</p><p className="account-note"><LocaleText>{"찜과 최근 본 장소는 자동 저장됩니다. 일정은 편집 후 일정 저장을 눌러 주세요. 같은 계정으로 다른 기기에서도 확인할 수 있습니다."}</LocaleText></p></div></section>
    {children}
