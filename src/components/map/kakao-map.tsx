...
const markerColors = { selected: "#5b4cf5", default: "#2386dd" };

export const KakaoMap = forwardRef<KakaoMapHandle, KakaoMapProps>(function KakaoMap({ appKey, initialCenter, places, selectedId, onReadyStateChange, onSelect }, ref) {
  ...
  useEffect(() => {
    ...
    markersRef.current = places.filter(hasValidCoordinates).map((place) => {
      ...
      const image = createMarkerImage(maps, selected ? markerColors.selected : markerColors.default);
  ...
  }, [onSelect, places, ready, selectedId]);
...
function createMarkerImage(maps: KakaoMaps, color: string) {
  ...
  return new maps.MarkerImage(source, new maps.Size(38, 46), { offset: new maps.Point(19, 44) });
}
