  ...
  return (
    <div className="map-explorer-layout">
      <section className={`map-explorer-canvas ${mapStatus === "ready" ? "has-live-map" : ""}`} aria-label="K-콘텐츠 촬영지 지도">
        <KakaoMap appKey={appKey} initialCenter={contents[0]?.coordinates ?? { latitude: 35.1532, longitude: 129.1186 }} places={mapPlaces} selectedId={selected?.id} onReadyStateChange={handleMapStatus} onSelect={(place) => setSelectedId(place.id)} />
