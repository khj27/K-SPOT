...
export default async function PlannerResultPage({ searchParams }: PlannerResultProps) {
  ...
  if (params.startDate || params.endDate) {
    ...
    catch { return <main className="planner-result-page"><h1>여행 날짜를 확인해 주세요.</h1><p>시작일부터 종료일까지 1~31일의 유효한 날짜를 선택해 주세요.</p><Link href="/planner">날짜 다시 선택하기</Link></main>; }
  }
  ...
  return (
    <main className="planner-result-page">
      ...
      <section className="planner-result-heading">
        <div><p className="kspot-eyebrow">MY ITINERARY</p><h1>{params.region ?? "전국"} {days === 1 ? "당일" : `${days - 1}박 ${days}일`} 추천 코스</h1><p>선택한 날짜별로 최대 3곳씩 제안합니다. 방문 날짜와 순서를 자유롭게 조정하세요.</p></div>
      ...
      <p className="planner-demo-note">등록된 공개 콘텐츠를 이용한 추천입니다. 후보가 부족한 날은 자유 일정으로 표시합니다. 이동 수단과 동행자 정보는 일정에 기록되며, 이동 시간과 영업시간은 아직 추천에 반영되지 않습니다.</p>
    </main>
  );
}
