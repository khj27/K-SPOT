...
export default async function PlannerResultPage({ searchParams }: PlannerResultProps) {
  ...
  return (
    <main className="planner-result-page">
      ...
      <section className="planner-route-summary"><AppIcon name="route" size={22} /><div><strong>{rankedPlaces.length}<LocaleText>{"개의 촬영지 후보"}</LocaleText></strong><span><LocaleText>{"선택한 지역과 콘텐츠 취향을 반영한 추천"}</LocaleText></span></div><Link href="/mypage#saved-places"><LocaleText>{"저장한 장소 보기 "}</LocaleText><AppIcon name="arrow" size={15} /></Link></section>
