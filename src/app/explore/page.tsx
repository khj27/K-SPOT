...
type ExplorePageProps = {
  searchParams: Promise<{ q?: string; type?: string; region?: string; content?: string; page?: string }>;
};
...
export default async function ExplorePage({ searchParams }: ExplorePageProps) {
  ...
  const contents = exploreContents.filter((content) => {
    ...
    const matchesRegion = selectedRegion === "전체 지역" || normalizeRegion(content.region) === selectedRegion;
    return matchesContent && matchesQuery && matchesType && matchesRegion;
  });

  const pagination = paginate(contents, params.page);
  return (
    <main className="explore-page">
      ...
      <Form key={`${query}:${selectedType}:${selectedRegion}`} className="explore-filter-panel" action="/explore">
      ...
      <Pagination {...pagination} query={{ q: query, type: selectedType, region: selectedRegion, content: selectedContentId }} />
