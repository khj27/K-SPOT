...
export default async function HomePage() {
  ...
  return (
    <main className="home-dashboard"><section className="home-intro"><p className="kspot-eyebrow">YOUR K-CONTENT JOURNEY</p><h1><LocaleText>내 취향의 촬영지에서 나만의 여행까지</LocaleText></h1><p><LocaleText>지도에서 장소를 찾고, 추천 코스를 편집해 저장하세요. 마음에 든 장소는 마이페이지에 모아둘 수 있어요.</LocaleText></p><div className="ui-actions"><Link className="kspot-primary-button" href="/planner"><LocaleText>새 일정 만들기</LocaleText></Link><Link className="ui-button" href="/suggest"><LocaleText>콘텐츠 제보</LocaleText></Link></div></section>
      {contents.length > 0 && <section aria-label={t("등록 콘텐츠 지도")}><MapExplorer appKey={process.env.NEXT_PUBLIC_KAKAO_MAP_KEY ?? ""} contents={contents} /></section>}<section><h2 aria-hidden="true">&nbsp;</h2><div className="explore-result-grid">{contents.slice(0, 6).map((content) => <article className="explore-content-card" key={content.id}><Link href={`/spots/${content.id}`}><div className={`explore-card-visual visual-${content.visual}`}><ContentThumbnail src={content.imageUrl} title={content.title} /></div><div className="explore-card-body"><p>{content.region} · <LocaleText>{content.type}</LocaleText></p><h2>{content.title}</h2><span>{content.spotName}</span></div></Link></article>)}</div>{!contents.length && <p><LocaleText>{"공개된 콘텐츠를 준비하고 있습니다."}</LocaleText></p>}</section>
