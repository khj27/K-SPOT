...
export default async function MyPage() {
  ...
  return <main className="saved-page"><section className="saved-heading"><div><p className="kspot-eyebrow">MY K-SPOT</p><h1><LocaleText>{"나의 K-SPOT 여행"}</LocaleText></h1><p><LocaleText>{"저장한 장소와 일정, 최근 둘러본 곳을 한눈에 확인하세요."}</LocaleText></p></div></section><AccountPanel user={user}><TravelDataBoundary><TravelDashboard contents={contents} /><section id="saved-places"><h2><LocaleText>찜한 장소</LocaleText></h2><SavedSpotsList contents={contents} /><SavedTourPlaces /></section></TravelDataBoundary><Link className="ui-button" href="/suggest"><LocaleText>콘텐츠 제보</LocaleText></Link></AccountPanel></main>;
}
