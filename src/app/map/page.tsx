...
export default async function MapPage({ searchParams }: MapPageProps) {
  ...
  const regions = ["전체 지역", ...new Set([...exploreRegions.filter((region) => region !== "전체 지역"), ...exploreContents.map((content) => normalizeRegion(content.region))])];
  ...
  return (
    <main className="map-page">
      ...
      <form className="map-filter-panel" action="/map"><label><span>콘텐츠 유형</span><select name="type" defaultValue={selectedType}>{exploreTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label><label><span>지역</span><select name="region" defaultValue={selectedRegion}>{regions.map((region) => <option key={region} value={region}>{region}</option>)}</select></label><button className="kspot-primary-button" type="submit">필터 적용 <AppIcon name="arrow" size={16} /></button></form>
      {contents.length > 0 ? <MapExplorer appKey={process.env.NEXT_PUBLIC_KAKAO_MAP_KEY ?? ""} contents={contents} key={`${selectedType}:${selectedRegion}`} /> : <section className="map-empty"><AppIcon name="map" size={32} /><h2>조건에 맞는 촬영지가 없습니다.</h2><p>필터를 바꿔 다시 검색해 보세요.</p><a href="/map">전체 지도 보기</a></section>}
      <p className="map-data-notice">지도 좌표는 실제 위·경도를 사용하며, 관리자 공개 데이터는 출처와 검수일을 함께 관리합니다.</p>
    </main>
  );
}
